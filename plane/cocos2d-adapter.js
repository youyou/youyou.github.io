...
function create_table_view( size, cellHeight, gap, numRows, cellForRowFunc, cellSelectFunc) {
    ...
    function updateVisibleCells()
    {
        ...
        for (var i = numRows.length - 1; i >= 0; i--) {
            contains[i] = false;
        }
    ...
    tableView.touchEnded = function( x, y ) {
         ...
         if( !BounceBack() )
         {
            ...
            // 惯性运动
            tableView.runAction(Ease( ease.easeOutQuad, ChangeValue( Math.abs(v)/30, tableView.y_offset, v*15, function(v) {
