...
function SetColor(node, color) {
    node.color = formatColor(color);;
    node.element.style.backgroundColor = "rgba("+node.color.r+","+node.color.g+","+node.color.b+","+node.bgOpacity+")"; //color.toString();
}

function SetBgOpacity(node, opacity) {
    node.bgOpacity = opacity;
    if( typeof(node.color) !== "undefined" ) {
        SetColor(node, node.color);
    }
}
...
// TODO:: 
// perfermance boost by cache position info in node tree

function GetBoundingRectInWorld(node)
{
    var x = node.x - node.width*node.ax*node.scalex;
    var y = node.y - node.height*node.ay*node.scaley;

    var curNode = node;

    while( curNode.parent ) {
        curNode = curNode.parent;
        x += (curNode.x - curNode.width*curNode.ax*curNode.scalex);
        y += (curNode.y - curNode.height*curNode.ay*curNode.scaley);
    };

    return CCRectMake( x, y, node.width*node.scalex, node.height*node.scaley);
}
...
function SetTouchEnabled(node, bEnabled) {
    ...
    function getXYFromEvent(e)
    {
        ...
        var rect = GetBoundingRectInWorld(node);
    ...
    function down(e) {
        ...
        if( pervent && e && typeof (e.preventDefault) !== 'undefined')
           e.preventDefault();
        ...
        if (Sys.mobileSafari) {
            ...
            node.element.addEventListener("touchcancel", cancel, false);
        }
        else if (IS_IE) {
    ...
    function stop(e) {
        ...
        if (Sys.mobileSafari) {
            ...
            node.element.removeEventListener("touchcancel", cancel, false);
        }
        else {
...
/*
function SetTouchEnabled( node, bEnabled, bPreventDefault)
{    
    var inCycle = false;
    var x = 0;
    var y = 0;
    ...
    if( typeof(bPreventDefault) === "undefined" )
        node.disableDefaultTouchEvent = true;
    else
        node.disableDefaultTouchEvent = bPreventDefault;
    ...
    function down(e) {
        ...
        if ( node.disableDefaultTouchEvent && e && typeof (e.preventDefault) !== 'undefined' )
            e.preventDefault();
...
function CCNode_instance(element_type) {
    ...
    if (str != "") {
        ...
        img.onload = function () {
            ...
            if (!IS_IE) {
                ...
                parent_instance.element.src = str;
            } else {
...
function CCSprite_index() {
    ...
    this.createWithSpriteFrame = function (texture, frame) {
        ...
        if (shouldCatchImage()) {
            ...
            if (sprite) {
                ...
                //cclog("catche missed!");
                sprite = CCSpriteFromFrame_instance(new CCNode_instance("img"), texture, frame);
    ...
    this.create = function (str) {
        if (getSpriteFrameInfo(str) == null) {
            return CCSprite_instance(new CCNode_instance("img"), str);
        } else {
...
function CCLabel_instance(parent_instance, str) {
    ...
    parent_instance.setScale = function (scale) {

        var anchorX = parseInt(parent_instance.width * parent_instance.ax);
        var anchorY = parseInt(parent_instance.height * parent_instance.ay);

        parent_instance.transform.scalex = parent_instance.scaley = scale;
        parent_instance.transform.scalex = parent_instance.scaley = scale;

        parent_instance.element.style["transform-origin"] = anchorX + "px " + anchorY + "px";
        parent_instance.element.style.transform = "scale(" + scale + ")";
...
function createButton(callback, imagename) {
    ...
    btn._originalScaleX = btn.scale;
    btn._originalScaleY = btn.scale;

    btn._setScale = btn.setScale;
    btn.setScale = function (scale) {
        btn._setScale(scale);
        btn._originalScaleX = scale;
        btn._originalScaleY = scale;
    };

    btn._setScaleX = btn.setScaleX;
    btn.setScaleX = function (scalex) {
        btn._setScaleX(scalex);
        btn._originalScaleX = scalex;
    };

    btn._setScaleY = btn.setScaleY;
    btn.setScaleY = function (scaley) {
        btn._setScaleY(scaley);
        btn._originalScaleY = scaley;
    };

    function up() {
        btn._setScaleX(btn._originalScaleX);
        btn._setScaleY(btn._originalScaleY);
    }

    btn.touchBegin = function () {
        btn._setScaleX(btn._originalScaleX * 1.2);
        btn._setScaleY(btn._originalScaleY * 1.2);
    };

    btn.touchEnded = function onUp() {
        up();
...
function FadeTo(time, opacity, isBgOpacity=false) {
    var obj = TimeAction(time);
    function start(target) {
        obj._start(target);
        if(isBgOpacity) {
            obj.sOpacity = target.bgOpacity;
            obj.dOpacity = opacity - obj.sOpacity;
            obj.step = function(p) {
                obj.target.setBgOpacity(obj.sOpacity + obj.dOpacity * p);
            }
        } else {
...
function TintTo(time, c, isBgColor=false) {
    ...
    function start(target) {
        ...
        if (typeof (target.color) !== 'undefined') {
            ...
            if (typeof (target.setBgColor) !== "undefined" && isBgColor == true) {
                obj.step = function (p) {
...
function StartAnimation(dt) {
    ...
    var useRequestAnimationFrame = false; //typeof (window.requestAnimationFrame) !== "undefined";
    ...
    if (typeof (window.performance) !== "undefined") {
        ...
        cclog("using performance.now() as time function");
        ...
        cclog("using (new Date).getTime() as time function");
    }

    if (useRequestAnimationFrame) {
        cclog("using window.requestAnimationFrame() to invoke update function");
        requestAnimationFrame(update);
    }
    else {
        cclog("using setInterval() to invoke update function");
...
function InitNodeRender(node) {
    ...
    node._gl_render = function () {
        ...
        var modelViewMatrix =
            [Math.cos(node.rotation_rad) * node.scalex, Math.sin(node.rotation_rad), 0, node.x,
